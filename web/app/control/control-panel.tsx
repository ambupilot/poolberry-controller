...
const blinkStyle = { animation: "poolberryCollectorBlink 0.8s ease-in-out infinite alternate" } as const;

export default function ControlPanel({ initialState, initialMode }: { initialState: OutputState; initialMode: ControllerMode }) {
  const [state, setState] = useState(initialState); const [mode, setMode] = useState(initialMode); const [busy, setBusy] = useState(false); const [message, setMessage] = useState<string | null>(null); const [collectorTransition, setCollectorTransition] = useState<CollectorTransition>(null);
  ...
  async function operation(kind: "filterpump" | "heatpump" | "collector", action: string) {
    ...
    catch (error) { if (transition) setCollectorTransition(null); setMessage(error instanceof Error ? error.message : "Opdracht kon niet worden verstuurd"); }
    finally { setBusy(false); }
  }

  async function stop() {
    setBusy(true); setMessage(null); setCollectorTransition(null);
  ...
  return <>
    <style>{`@keyframes poolberryCollectorBlink { from { opacity: 1; } to { opacity: .28; } }`}</style>
