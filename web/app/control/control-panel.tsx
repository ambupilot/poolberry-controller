...
export default function ControlPanel({ initialState, initialMode }: { initialState: OutputState; initialMode: ControllerMode }) {
  ...
  return <>
    ...
    <section className="panel"><h2>Programma&apos;s</h2><div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 12 }}>{programs.map((program) => <button key={program} type="button" disabled style={{ ...base, background: "rgba(37, 99, 235, 0.35)", cursor: "not-allowed" }}><span style={{ display: "block" }}>{program}</span><span style={{ display: "block", marginTop: 4 }}>UIT</span></button>)}</div></section>
