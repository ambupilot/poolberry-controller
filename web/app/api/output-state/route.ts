...
export async function GET() {
  ...
  try {
    const response = await fetch(`${API_URL}/internal/v1/devices/${DEVICE_ID}/output-state`, {
      cache: "no-store",
    });
    if (!response.ok) {
      return NextResponse.json({ detail: "Output state unavailable" }, { status: response.status });
    }
