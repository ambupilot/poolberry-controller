...
def authorize_device(device_id: str, authorization: str | None) -> None:
    if not DEVICE_ID or not DEVICE_TOKEN_SHA256: raise HTTPException(status_code=503, detail="Device authentication is not configured")
    if device_id != DEVICE_ID: raise HTTPException(status_code=401, detail="Unauthorized device")
    scheme, separator, token = (authorization or "").partition(" ")
    if separator != " " or scheme.lower() != "bearer" or not token: raise HTTPException(status_code=401, detail="Missing bearer token")
    supplied_hash = hashlib.sha256(token.encode("utf-8")).hexdigest()
    if not hmac.compare_digest(supplied_hash, DEVICE_TOKEN_SHA256): raise HTTPException(status_code=401, detail="Invalid device token")


def normalize_output_id(output_id: str) -> str:
    normalized = output_id.upper()
    if normalized not in SUPPORTED_OUTPUTS: raise HTTPException(status_code=404, detail="Unknown output")
    return normalized


def normalize_command_id(command_id: str) -> str:
    normalized = command_id.upper()
    if normalized not in SUPPORTED_COMMANDS: raise HTTPException(status_code=404, detail="Unknown command")
    return normalized


def device_status(device: Device) -> str:
    now = datetime.now(timezone.utc); last_seen = device.last_seen
...
def get_or_create_config(db: Session, device_id: str) -> DeviceConfig:
    ...
    if config is None: config = DeviceConfig(device_id=device_id); db.add(config); db.commit(); db.refresh(config)
    return config


def get_or_create_mode(db: Session, device_id: str) -> ControllerMode:
    mode = db.get(ControllerMode, device_id)
    if mode is None: mode = ControllerMode(device_id=device_id, mode="NORMAL"); db.add(mode); db.commit(); db.refresh(mode)
    return mode


def config_response(config: DeviceConfig) -> DeviceConfigResponse:
    return DeviceConfigResponse(device_id=config.device_id, flow_f1_pulses_per_liter=config.flow_f1_pulses_per_liter, flow_f2_pulses_per_liter=config.flow_f2_pulses_per_liter, filter_flow_safety_bypass=config.filter_flow_safety_bypass, filter_min_flow_lph=config.filter_min_flow_lph, filter_flow_grace_seconds=config.filter_flow_grace_seconds, updated_at=config.updated_at)
...
def set_command(db: Session, device_id: str, command_id: str, enabled: bool, now: datetime) -> OutputCommand:
    ...
    if command is None: command = OutputCommand(device_id=device_id, output_id=command_id, enabled=enabled, pending=True, updated_at=now); db.add(command)
    else: command.enabled = enabled; command.pending = True; command.updated_at = now
    return command


def clear_pending(db: Session, device_id: str, now: datetime) -> None:
    for command in db.scalars(select(OutputCommand).where(OutputCommand.device_id == device_id, OutputCommand.pending.is_(True))).all(): command.pending = False; command.updated_at = now


def queue_stop(db: Session, device_id: str, now: datetime) -> OutputCommand:
    clear_pending(db, device_id, now); return set_command(db, device_id, "STOP", False, now)


def queue_operation(db: Session, device_id: str, command_id: str, now: datetime) -> OutputCommand:
    clear_pending(db, device_id, now); return set_command(db, device_id, command_id, True, now)


def require_normal(db: Session, device_id: str, label: str) -> None:
...
def update_internal_config(device_id: str, payload: DeviceConfigUpdate, db: Session = Depends(get_db)):
    if db.get(Device, device_id) is None: raise HTTPException(status_code=404, detail="Device not found")
    config = get_or_create_config(db, device_id); config.flow_f1_pulses_per_liter = payload.flow_f1_pulses_per_liter; config.flow_f2_pulses_per_liter = payload.flow_f2_pulses_per_liter; config.filter_flow_safety_bypass = payload.filter_flow_safety_bypass; config.filter_min_flow_lph = payload.filter_min_flow_lph; config.filter_flow_grace_seconds = payload.filter_flow_grace_seconds; config.updated_at = datetime.now(timezone.utc); db.commit(); db.refresh(config); return config_response(config)
...
def update_controller_mode(device_id: str, payload: ControllerModeUpdate, db: Session = Depends(get_db)):
    if db.get(Device, device_id) is None: raise HTTPException(status_code=404, detail="Device not found")
    mode = get_or_create_mode(db, device_id); now = datetime.now(timezone.utc); target = payload.mode.upper()
    if target == "MANUAL": clear_pending(db, device_id, now)
    elif target == "NORMAL": queue_stop(db, device_id, now)
    mode.mode = target; mode.updated_at = now; db.commit(); db.refresh(mode); return mode_response(mode)
...
def stop_all_outputs(device_id: str, db: Session = Depends(get_db)):
    if db.get(Device, device_id) is None: raise HTTPException(status_code=404, detail="Device not found")
    now = datetime.now(timezone.utc); command = queue_stop(db, device_id, now); db.commit(); db.refresh(command); return command_response(command)
...
def filterpump_on(device_id: str, db: Session = Depends(get_db)):
    ...
    now = datetime.now(timezone.utc); command = queue_operation(db, device_id, "FILTERPUMP_ON", now); db.commit(); db.refresh(command)
...
def set_output_command(device_id: str, output_id: str, payload: OutputCommandRequest, db: Session = Depends(get_db)):
    if db.get(Device, device_id) is None: raise HTTPException(status_code=404, detail="Device not found")
    if get_or_create_mode(db, device_id).mode != "MANUAL": raise HTTPException(status_code=409, detail="Direct output commands require MANUAL mode")
    output_id = normalize_output_id(output_id); now = datetime.now(timezone.utc); command = set_command(db, device_id, output_id, payload.enabled, now); db.commit(); db.refresh(command); return command_response(command)
...
def get_next_command(device_id: str, authorization: str | None = Header(default=None), db: Session = Depends(get_db)):
    authorize_device(device_id, authorization); command = db.scalar(select(OutputCommand).where(OutputCommand.device_id == device_id, OutputCommand.pending.is_(True)).order_by(OutputCommand.updated_at.asc()).limit(1)); return None if command is None else command_response(command)
...
def acknowledge_command(device_id: str, command_id: str, authorization: str | None = Header(default=None), db: Session = Depends(get_db)):
    authorize_device(device_id, authorization); command_id = normalize_command_id(command_id); command = db.get(OutputCommand, (device_id, command_id))
    if command is None: raise HTTPException(status_code=404, detail="Command not found")
    command.pending = False; command.updated_at = datetime.now(timezone.utc); db.commit(); db.refresh(command); return command_response(command)
...
def heartbeat(device_id: str, payload: HeartbeatRequest, authorization: str | None = Header(default=None), db: Session = Depends(get_db)):
    ...
    if device is None: device = Device(device_id=device_id, firmware_version=payload.firmware_version, last_seen=now, uptime_seconds=payload.uptime, wifi_connected=payload.wifi_connected); db.add(device)
    else: device.firmware_version = payload.firmware_version; device.last_seen = now; device.uptime_seconds = payload.uptime; device.wifi_connected = payload.wifi_connected
